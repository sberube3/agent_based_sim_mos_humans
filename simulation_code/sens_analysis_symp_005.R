...
#changes around 221, 270, 316, 340
#more changes around 238, 359
...
# md3 <- c()
# n_rep <- 1000
# for(i in c(2, 5, 8, 10, 14, 16)){
#   temp1 <- rep(NA, n_rep)
#   temp2 <- rep(NA, n_rep)
#   for(j in 1:n_rep){
#     temp1[j] <- get_mos_death(i)
#     temp2[j] <- get_mos_death2(i)
#   }
#   
# 
#   md1[length(md1) + 1] <- mean(temp1)
#   md2[length(md2) + 1] <- mean(temp2)
#   md3[length(md3) + 1] <- mean(get_mos_death3(rep(i, n_rep)))
# }
# 
# plot(md1, md2)
# abline(0, 1)
...
# print(microbenchmark(sapply(1:1000, get_mos_death)))
# print(microbenchmark(sapply(1:1000, get_mos_death2)))
# print(microbenchmark(get_mos_death3(1:1000)))
...
run_biting_sim<- function(pr_symp, pr_clear, pr_off_feed, pr_on_feed, pr_hum_to_mos, pr_mos_to_hum,
  ...
  for(q in 1:n_sim){
    ...
    symp_index<- rbinom(n_p,1,pr_symp)
    

    symp_age<-rep(0,n_p)
    
    inf_p_with_old_hap<- get_old_p_infec(age_haps_p)
    
    symp_age[which(symp_index==1&inf_p_with_old_hap==1)]<- 1
    ...
    for(r in 1:722){
      ...
      age_haps_p[which(symp_age==2),]<-0
              ...
              old_infec_p<-get_old_p_infec(age_haps_p)
              
              symp_age<- ifelse(symp_index==1&old_infec_p==1&symp_age==0, 1, symp_age)
            #}
          #}
        #}
